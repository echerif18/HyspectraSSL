...
for i, (db_lb_all, samples_val_ext, test_ids) in islice(enumerate(sliding_custom_cv(db_lb, seed=42)), 0, None):
    run = 'Gan_NoRTM_{}_gp{}UNlabels_{}'.format(formatted_datetime, i, seed)
    ...
    ### external
    ext_val_x, ext_val_y = data_prep_db(samples_val_ext, ls_tr, weight_sample=False)
    meta_ext = samples_val_ext.iloc[:, :8] 
    ...
    settings_dict = {
        ...
        'input_shape': input_shape, #1720, 500
        'type':type_s,
        'latent_dim': 100,
        'learning_rate': lr,
        'weight_decay': 1e-4,
